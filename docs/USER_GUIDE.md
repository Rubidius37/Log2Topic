...
이 문서는 Log2Topic의 편집기 설정과 평소 사용 방법을 자세히 설명합니다. 처음 실행하는 중이라면 먼저 [README의 처음 시작하기](../README.md#처음-시작하기)를 따라 첫 분류를 완료하세요.

[README로 돌아가기](../README.md) · [분류 규칙](../Workspace/Classification_Rules.md) · [Notion 연동](NOTION_INTEGRATION.md)

## 작업공간 준비

Log2Topic은 최상위 프로젝트 폴더에 앱 파일을 두고, `Workspace/` 안에 모든 Markdown 작업물을 둡니다. 편집기에서는 `Workspace/`를 열고, 아래에 나오는 콘텐츠 경로는 그 폴더 기준으로 이해하면 됩니다.
...
Obsidian에서는 **새 Vault 만들기**가 아니라 **폴더를 Vault로 열기**를 선택하고, `Workspace/` 폴더를 지정합니다. `Daily_Logs/`만 따로 Vault로 열면 생성 문서의 상대 링크가 올바르게 연결되지 않습니다.
...
- 자세한 복구 절차는 [Notion 연동 안내](NOTION_INTEGRATION.md)를 참고합니다.
