...
작업 일지는 날짜별로 쓰는 것이 자연스럽지만, 한 주제의 결정과 시도, 결과는 여러 날짜에 흩어집니다. 나중에 그 흐름을 다시 읽으려면 일지를 하나씩 뒤져야 했습니다. 같은 내용을 주제별 노트에 다시 옮겨 적으면 기록이 중복되고 서로 어긋날 수도 있습니다.

그래서 **직접 쓴 일지를 원본으로 두고, 한 번 쓴 기록을 주제별로 다시 읽는 도구**를 만들었습니다. Log2Topic은 사용자가 정한 Heading과 분류 규칙에 따라 문서를 생성하고, Source ID와 링크로 각 내용을 원본에 연결합니다. 기록의 의미를 판단하는 일은 작성자에게 남겨두고, 프로그램은 이미 남긴 기록을 다시 살펴보기 쉽게 배열합니다.
